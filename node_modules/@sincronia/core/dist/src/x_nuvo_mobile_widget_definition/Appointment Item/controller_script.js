(function () {
  return {
    ...
    controller: function ($scope, MobiusRecord, ViewProvider, $filter) {
      ...
      function convertUTCToLocal(dateStr, fmt) {
        try {
          return window.moment.utc(dateStr).local().format(fmt);
        } catch (e) {
          return '???';
        }
      }
      ...
      function toLocalTimeString(dateStr) {
        return convertUTCToLocal(dateStr, 'LT');
      }

      function toLocalDateString(dateStr) {
        return convertUTCToLocal(dateStr, 'll');
      }

      var woClassMap = JSON.parse(Mobius.getProperty('eamWorkOrderViewMap'));

      var startDateTime = $scope.current.event_start.value;
      var endDateTime = $scope.current.event_end.value;

      $scope.startTime = toLocalTimeString(startDateTime);
      $scope.endTime = toLocalTimeString(endDateTime);
      $scope.date = toLocalDateString(startDateTime);
